                        ...
                        <div class="table-responsive">
                            <table id="users-table" class="table table-hover" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    {{-- {{
                                        $st = \App\Models\Materiel::find($stock->materiels_id)

                                    }} --}}
                                    @php
                                        $total = \App\Http\Controllers\StockactuelController::nombre($stock->materiels_id);
                                        $perte = \App\Http\Controllers\PerteController::nombre($stock->materiels_id);
                                        $loc = \App\Http\Controllers\LocationController::nombre($stock->materiels_id);
                                    @endphp
                                        <tr>
                                            {{-- @forelse (\App\Models\Materiel::find($stock->materiels_id) as $item) --}}
                                            <td>{{$stock->id}}</td>
                                            <td>{{$stock->materiels->nom}}</td>
                                            <td>{{$total - $perte - $loc}}</td>
                                            {{-- @empty
                                                
                                            @endforelse --}}
                                            {{-- <td>{{$stock->id}}</td>
                                            <td>{{$stock->materiels_id}}</td>
                                            <td>{{$stock->nombre}}</td> --}}
                                            {{-- <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
                                                            <form class="f-del" action="{{route('stock.edit', $stock->id)}}" method="GET">
                                                                <input type="submit" class="dropdown-item" value="Modifier">
                                                            </form>
                                                            <form class="f-del" action="{{route('stock.destroy',$stock->id)}}" method="POST">
                                                                @csrf
                                                                @method('delete')
